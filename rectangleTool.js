function rectangleTool() {
  ...
  /*to smoothly draw we'll draw a rectangle from the start mouse location
  ...
  this.draw = function () {
    ...
    if (mouseIsPressed) {
      //draw filled or notfilled rectangle
      if (dropbox.value() == "Filled") {
        fill(colourP.selectedColour);
      } else {
        noFill();
      }
      //check if previousMouseX and previousMouseY are -1. Set them and startMouseX and Y to the current
  ...
  //add the dropbox with options to fill or not fill the rectangle
